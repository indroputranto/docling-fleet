...
class DynamicTextExtractor:
    ...
    def _organize_charter_party_clauses(self, charter_party_content: List[str]) -> tuple[Dict[str, Dict[str, Any]], List[str]]:
        ...
        clause_headers = [
            ...
            'Victualing and Expenses', 'Hire Payment', 'Speed and Consumption', 'Spaces Available',
        ...
        for line in charter_party_content:
            ...
            # Second, check for numbered patterns like "2) Delivery" or "3) Redelivery" 
                ...
                if tab_clause_match:
                    ...
                    title_length_ok = len(clause_title) > 3
                    if (len(clause_title) <= 85 and ':' not in clause_title and 
                        (header_match or title_length_ok)):
                        is_clause_header = True
                        # Save the previous clause BEFORE updating variables
                        if current_clause_number and current_clause_title:
                            clause_key = f"clause_{current_clause_number}"
                ...
                # Third, check for standalone section headers
                    ...
                    if len(line_stripped) <= 85 and ':' not in line_stripped:
                        # This is a section header - treat as clause delimiter
                        is_clause_header = True
                        # Save the previous clause BEFORE updating variables
                        if current_clause_number and current_clause_title:
                            clause_key = f"clause_{current_clause_number}"
                        ...
                        current_clause_content = []  # Reset for new clause
                        clause_counter += 1
                        logging.info(f"Found section Charter Party clause {current_clause_number}: {current_clause_title[:50]}...")
    ...
    def _extract_misplaced_communication_details(self, chapters: Dict[str, Any]) -> None:
        ...
        for chapter_key, chapter_data in chapters.items():
            if chapter_key == "7_vessel_communication_details":
                continue  # Skip the target chapter
            ...
            for i, line in enumerate(content):
                ...
                # Check if this line starts a communication details section
                if re.match(r'^\d+\.?\s*vessels?\s+communication\s+details?', line_lower):
        ...
        if communication_content:
            chapters["7_vessel_communication_details"]["content"] = communication_content
            logging.info(f"Moved {len(communication_content)} lines to vessel communication details chapter")
